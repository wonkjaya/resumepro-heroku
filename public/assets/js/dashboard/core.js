$(function(){
  ...
  function toggleSideBar(_this){
    ...
    if(w.hasClass("sb-collapsed")){
      ...
      $.cookie('sidebar-cookie','open',{expires:365, path:'/'});
    }else{
      $(".fa",b).removeClass("fa-angle-left").addClass("fa-angle-right");
      w.addClass("sb-collapsed");
      $.cookie('sidebar-cookie','closed',{expires:365, path:'/'});
    }
  }
  ...
  if ($("#div-wrapper").hasClass("fixed-menu")){
      $('#head-nav').addClass('navbar-fixed-top');
  }
  function updateHeight(){
    if(!$("#div-wrapper").hasClass("fixed-menu")){
      var button = $("#div-wrapper .collapse-button").outerHeight();
      var navH = $("#head-nav").height();
      //var document = $(document).height();
      var cont = $("#pcont").height();
      var sidebar = ($(window).width() > 755 && $(window).width() < 963)?0:$("#div-wrapper .menu-space .content").height();
      ...
      $("#div-wrapper .menu-space").css("min-height",height);
    }else{
      $("#div-wrapper .nscroller").nanoScroller({ preventPageScrolling: true });
    }
  }
        

      /*VERTICAL MENU*/
      $(".div-vnavigation li ul").each(function(){
        $(this).parent().addClass("parent");
      });
      
      $(".div-vnavigation li ul li.active").each(function(){//If li.active is a sub-menu item open all its parents
      ...
      $('.div-vnavigation li > a').on('click', function (e) {
        ...
        if ($('#div-wrapper').hasClass('sb-collapsed') === false){
    ...
    if ($('#div-wrapper').hasClass('sb-collapsed')) {
        var elem = $('.div-sidebar ul');
    ...
    var handleSidebarAndContentHeight = function () {
      ...
      if($("#div-wrapper").hasClass("fixed-menu")){
        var scroll =  $("#div-wrapper .menu-space");
        scroll.addClass("nano nscroller");
 
        function update_height(){
          var button = $("#div-wrapper .collapse-button");
      ...
      /*SubMenu hover */
        ...
        function showMenu(_this, e){
          if(($("#div-wrapper").hasClass("sb-collapsed") || ($(window).width() > 755 && $(window).width() < 963)) && $("ul",_this).length > 0){   
        ...
        $(".div-vnavigation li").hover(function(e){
          showMenu(this, e);
        },function(e){
          tool.removeClass("over");
          setTimeout(function(){
            if(!tool.hasClass("over") && !$(".div-vnavigation li:hover").length > 0){
              tool.hide();
            }
          },500);
        });
        ...
        $(".div-vnavigation li").click(function(e){
          if((($("#div-wrapper").hasClass("sb-collapsed") || ($(window).width() > 755 && $(window).width() < 963)) && $("ul",this).length > 0) && !($(window).width() < 755)){
            showMenu(this, e);
            e.stopPropagation();
          }
        });
  ...
  $(function(){
    if($('body').hasClass('animated')){
      $("#div-wrapper").css({opacity:1,'margin-left':0});
    }
    ...
    if($.cookie('sidebar-cookie') && $.cookie('sidebar-cookie') == 'closed'){
        $('#div-wrapper').addClass('sb-collapsed');
